...
from src.UI.base_class_pages import BaseWindow
# from src.UI.pages import settings_page
from src.UI import plot_frame_layout, text_layout
...
class StartPage(BaseWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Лог данных
        self.log_text = text_layout.LogText(self.tool_frame, height=int(self.y/2), width=45)
        ...
        self.plot_drawer = plot_frame_layout.PlotHelper(self.data_storage, self.log_text, self.canvas, self.fig)

        self.start_action()
        
    def update_plot(self):
        if not self.prog_state.is_reconect:
            self.plot_drawer.update_plot()
            self.fn_repeat_id1 = self.after(1000, self.update_plot)
        else:
            self.log_text.insert(tk.END, self.log_formats + "Попытка переподключить устройство\n", "red")
            self.fn_repeat_id1 = self.after(2500, self.update_plot)
    ...
    def start_action(self):
        if not self.process_running or not self.settings:
            ...
            # self.prog_state.init_usb_thread()
            self.update_plot()
